from documents.documents import Query, AgentResponse, Document
        ...
        try:
            ...
            for collection_info in self.collections:
                ...
                result = collection.query(
                    query_texts=[query.text], n_results=query.top_k
                )

                for i, doc_id in enumerate(result["ids"][0]):
                    # Create base metadata
                    metadata = {"id": doc_id, "collection": collection_info.name}
                    ...
                    if "metadatas" in result and result["metadatas"][0][i]:
                        metadata.update(result["metadatas"][0][i])

                    document = Document(
                        content=result["documents"][0][i], metadata=metadata
                    )
