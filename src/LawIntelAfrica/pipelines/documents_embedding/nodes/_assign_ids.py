...
def assign_unique_ids(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["chunk_index"] = df.groupby(["document_name", "page_label"]).cumcount()  # Create chunk index per page
    df["chunk_id"] = df["document_name"] + ":" + df["page_label"].astype(str) + ":" + df["chunk_index"].astype(str)
