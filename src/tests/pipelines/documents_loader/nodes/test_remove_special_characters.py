...
class Test_remove_characters:
    def test_remove_special_characters(self):
        ...
        # Arrange
        text = [
            "Ils dansèrent sous la lune brillante, accompagnés par le doux chant des grillons. 🌳🌕"
        ]
        df = pd.DataFrame(text, columns=["text"])

        expected_text = [
            "Ils danserent sous la lune brillante, accompagnes par le doux chant des grillons. "
        ]
