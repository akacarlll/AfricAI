...
def get_page_title(title: str) -> str:
    ...
    cleaned_title = ''.join([c for c in normalized_title if not unicodedata.combining(c)])
...
def save_file(file_path: str, url: str, text: str, title_on_page: str, page_label: int = 0, metadata: dict = None) -> None:
    
    with open(file_path, mode='w', newline='', encoding='utf-8') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(["URL", "text", "page_title", "page_label", "metadata"])
        writer.writerow([url, text, title_on_page, page_label, metadata])
